...
def expected_sqrt(mean):
    ...
    truncated_taylor_around_mean = np.sqrt(mean) - np.sqrt(mean) ** (
        -0.5) / 8 + np.sqrt(mean) ** (-1.5) / 16

    return nonzeros * (
            truncated_taylor_around_0 * (mean < 4) + truncated_taylor_around_mean * (
                mean >= 4))
...
if __name__ == "__main__":
    ...
    exp, class_labels, programs, lib_size, umis = simulate_classes(
        ...
        args.n_cells,
        args.n_genes,
        prog_kw=dict(scale=3. / np.sqrt(args.n_features), sparsity=1.0),
        class_kw=dict(scale=3. / np.sqrt(args.n_latent), sparsity=1.0),

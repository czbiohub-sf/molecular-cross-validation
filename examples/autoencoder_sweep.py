...
run_group = parser.add_argument_group("run", help="Per-run parameters")
run_group.add_argument("--seed", type=int, required=True)
run_group.add_argument("--gpu", type=int, required=True)

data_group = parser.add_argument_group("data", help="Input and output parameters")
...
model_group = parser.add_argument_group("model", help="Model parameters")
...
bottlenecks.extend(3 * b // 2 for v in bottlenecks[:-1])
bottlenecks.sort()
...
def test_bottlenecks(bs, m_factory, opt_factory, criterion, train_data, val_data):
    b_results = dict()
    scheduler_kw = {
        "t_max": 128, "eta_min": args.learning_rate / 100.0, "factor": 1.0
    }
...
with torch.cuda.device(device):
    ...
    optimizer_factory = lambda: AggMo(
        model.parameters(),
